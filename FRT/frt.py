...
class FRT(nn.Module):
	...
	def predict(self, src_indicies, src_padding_mask):
		...
		tgt_indicies = torch.zeros(1, src_indicies.shape[1], dtype=torch.long)							# (1, N)
		...
		for k in range(steps):
			...
			tgt_indicies = torch.cat((tgt_indicies, torch.zeros(1, src_indicies.shape[1], dtype=torch.long)))
